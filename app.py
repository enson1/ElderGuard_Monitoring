...
ASL_TRAIN_PATH_FOR_LABELS = r"C:\\Users\\yash\\Downloads\\asl_alphabet_train" # <--- CHANGE IF NEEDED for ASL
ASL_IMG_HEIGHT = 64
ASL_IMG_WIDTH = 64
...
class ClapSession(db.Model):
    ...
    user_id = db.Column(db.Integer, db.ForeignKey('fr_user.id'), nullable=True) # MODIFIED: Added user_id

    user = db.relationship('User', backref=db.backref('clap_sessions', lazy=True)) # MODIFIED: Added relationship

    def __repr__(self):
        return f'<ClapSession {self.id} - {self.clap_count} claps by User {self.user_id}>' # MODIFIED: repr
...
class FallEvent(db.Model):
    ...
    user = db.relationship('User', backref=db.backref('fall_events', lazy=True)) # MODIFIED: Added relationship

    def __repr__(self):
        return f'<FallEvent {self.id} at {self.timestamp} from {self.source} by User {self.user_id}>' # MODIFIED: repr
# --- >>> END NEW <<< ---
...
def initialize_asl_detector():
    """Initializes ONLY the SignDetector for ASL with detailed logging and error handling."""
    global asl_detector
    logging.info("Attempting to initialize ASL Detector...")
    try:
        ...
        # Add more detailed checks and logging after the call
        if asl_detector is not None:
            if asl_detector.model is not None:
                logging.info("ASL Detector and its Keras model initialized successfully.")
                if hasattr(asl_detector, 'labels') and asl_detector.labels:
                    logging.info(f"ASL Detector labels loaded: {len(asl_detector.labels)} labels. First few: {asl_detector.labels[:5]}")
                else:
                    logging.warning("ASL Detector initialized, model loaded, but labels seem to be empty.")
            else:
                logging.error("ASL Detector was instantiated, but its internal Keras model FAILED to load. "
                              f"Please check the ASL_MODEL_PATH ('{ASL_MODEL_PATH}') and ensure the model file is valid and accessible.")
        else:
            logging.error("ASL Detector FAILED to initialize (the 'asl_detector' object is still None). "
                          "This might indicate an issue within the SignDetector class constructor or critical path configurations not being found.")
    except Exception as e_asl_init:
        logging.error(f"An unhandled exception occurred during ASL Detector initialization: {e_asl_init}", exc_info=True)
...
def process_video_stream(stream_user_id=None): # MODIFIED: Accept stream_user_id
    ...
    frame_num = 0
    logging.info(f"Fall Detection: Webcam processing thread started for user_id: {stream_user_id}.")
    ...
    try:
        while camera_service.is_active_by('fall_detection'): # Loop while this user owns the camera
            ...
            if not is_currently_falling and fall_confirm_counter >= FALL_CONFIRM_FRAMES:
                 ...
                 # --- Save Snippet/Image for Attachments ---
                 ...
                 try:
                     with app.app_context(): 
                         new_fall_event = FallEvent(
                             ...
                             source="Live Webcam",
                             person_id="-1", 
                             snippet_filename=db_snippet_filename,
                             user_id=stream_user_id # MODIFIED: Use the passed user_id
                         )
                         db.session.add(new_fall_event)
                         db.session.commit()
                         # MODIFIED: Logging prefixes like "Fall Detection:"
                         logging.info(f"Fall Detection: Saved FallEvent to DB: ID {new_fall_event.id} from Live Webcam, UserID: {stream_user_id}, Snippet: {db_snippet_filename}") # MODIFIED LOG
        ...
        # Don't just update to "Error" if it's a camera specific error.
        # Let the stop_webcam route handle overall manager status if the camera itself failed.
        # processing_manager.update_status(status="Error", error=f"Runtime error: {e}") # Maybe remove this direct update
    finally:
        logging.info("Fall Detection: Webcam processing thread finishing.")
        with lock:
            output_frame = None 

        # MODIFIED: Explicitly set manager to Idle if this stream was the reason it wasn't idle,
        # AND no *file processing* is ongoing.
        if not camera_service.is_active_by('fall_detection'): # Confirms the camera was indeed released for fall_detection
            current_pm_status = processing_manager.get_specific_status_field("status")
            task_id = processing_manager.get_current_task_id()

            # Check if the system is busy due to *file processing* specifically
            is_file_processing_busy = (task_id is not None) and \
                                      (current_pm_status not in ["Idle", "Completed", "Error"])

            if not is_file_processing_busy and \
               not clap_tracker.is_active() and \
               not camera_service.is_active_by('asl'):
                logging.info(f"Fall Detection: Webcam stopped. No file processing, clap, or ASL active. Resetting ProcessingManager to Idle from status: {current_pm_status}")
                processing_manager.reset_to_idle_if_not_busy() # This should now work if status was 'Streaming'
                # If reset_to_idle_if_not_busy still doesn't work because status was 'Streaming' (which makes is_busy true),
                # then we need a more direct reset.
                if processing_manager.get_specific_status_field("status") == "Streaming": # If it was stuck on Streaming
                    processing_manager.update_status(
                        status="Idle", current_file=None, output_file=None,
                        error=None, task_id=None, fall_occurred_overall=False,
                        fall_snippets=[], analysis_duration_seconds=None, fall_event_count=0
                    )
                    logging.info("Fall Detection: Forced ProcessingManager status to Idle from 'Streaming'.")
            else:
                logging.info(f"Fall Detection: Webcam thread stopped. Processing manager status remains: {current_pm_status} due to other activities (FileProc: {is_file_processing_busy}, Clap: {clap_tracker.is_active()}, ASL: {camera_service.is_active_by('asl')}).")


# --- Database Helper Functions ---
def get_clap_history_stats():
    """Queries the database and calculates history stats for the logged-in user."""
    stats = {
        "last_session": None,
        "best_session": None,
        "weekly_avg": 0.0,
    ...
    if 'fr_username' in session: # Access session directly if in request context
        user = User.query.filter_by(username=session['fr_username']).first()
        if user:
            current_user_id = user.id
    
    if not current_user_id:
        logging.warning("get_clap_history_stats: No user logged in or user not found. Returning empty stats.")
        return stats # Return empty stats if no user or called without user context

    try:
        with app.app_context(): # Ensure app context if called from thread/non-request
             base_query = ClapSession.query.filter_by(user_id=current_user_id) # MODIFIED: Filter by user_id

             stats["total_sessions"] = base_query.count() # MODIFIED
             if stats["total_sessions"] == 0:
                 return stats

             last = base_query.order_by(desc(ClapSession.end_time)).first() # MODIFIED
             if last:
                 stats["last_session"] = {
             ...
             best = base_query.order_by(desc(ClapSession.clap_count)).first() # MODIFIED
             if best:
                 stats["best_session"] = {
             ...
             weekly_sessions = base_query.filter(ClapSession.end_time >= one_week_ago).all() # MODIFIED
             if weekly_sessions:
             ...
             all_sessions_for_streak = base_query.order_by(desc(ClapSession.end_time)).all() # MODIFIED
             ...
             total_cal_query = base_query.with_entities(func.sum(ClapSession.calories_burned)).scalar() # MODIFIED
...
def index():
    ...
    is_clap_tracker_active = clap_tracker.is_active() 
    ...
    try:
        ...
        initial_clap_status = {"target_claps": 100, "clap_count": 0, "active": False} # Ensure active default

    # --- New logic for profile and bell alert ---
    current_user_username = session.get('fr_username')
    current_user_initials = "GU" # Default for Guest User

    if current_user_username:
        user_obj = User.query.filter_by(username=current_user_username).first()
        if user_obj:
            names = user_obj.username.split()
            if len(names) > 1:
                current_user_initials = (names[0][0] + names[-1][0]).upper()
            elif names: # Single name
                current_user_initials = names[0][:2].upper() if len(names[0]) > 1 else names[0][0].upper()
        else: 
            current_user_username = None # User in session but not in DB
    
    last_fall_event_time_str = None
    last_fall_event = FallEvent.query.order_by(FallEvent.timestamp.desc()).first()
    if last_fall_event:
        last_fall_event_time_str = last_fall_event.timestamp.strftime("%Y-%m-%d %H:%M:%S UTC")
    # --- End new logic ---

    return render_template('index.html',
                           webcam_active=is_fall_detection_active,
                           clap_tracker_active=is_clap_tracker_active, # Pass this
                           ...
                           current_user_username=current_user_username,         # New
                           current_user_initials=current_user_initials,       # New
                           last_fall_event_time=last_fall_event_time_str)     # New
...
def start_webcam():
    ...
    if not cap_instance:
        ...
        processing_manager.update_status(status="Error", error="Failed to acquire camera for fall detection.") 
        return redirect(url_for('index'))

    processing_manager.update_status(status="Starting Webcam...", error=None) 

    # --- MODIFIED: Get current_user_id to pass to the thread ---
    current_user_id_for_stream = None
    if 'fr_username' in session:
        user = User.query.filter_by(username=session['fr_username']).first()
        if user:
            current_user_id_for_stream = user.id
            logging.info(f"Fall detection stream starting for user ID: {current_user_id_for_stream}")
        else:
            logging.warning(f"Fall detection stream starting, but user '{session['fr_username']}' not found in DB. Events will be unassigned.")
    else:
        logging.info("Fall detection stream starting, no FR user logged in. Events will be unassigned.")
    # --- END MODIFICATION ---

    webcam_thread = threading.Thread(target=process_video_stream, args=(current_user_id_for_stream,), daemon=True) # MODIFIED: Pass user_id
    webcam_thread.start()
...
def stop_clap_tracker():
    ...
    if clap_tracker.is_active(): # Relies on ClapTracker's updated is_active
        ...
        current_user_id = None # MODIFIED: Variable to hold user_id
        if 'fr_username' in session:
            user = User.query.filter_by(username=session['fr_username']).first()
            if user:
                current_user_id = user.id
                logging.info(f"Clap session will be associated with user ID: {current_user_id}")
            else:
                logging.warning(f"User '{session['fr_username']}' not found in DB for clap session. Session will be unassigned.")
        else:
            logging.info("No FR user logged in. Clap session will be unassigned.")
            ...
            if count > 0 or duration > 5: # Only save meaningful sessions
                 with app.app_context(): # Need app context for db operations outside request
                    new_session = ClapSession(
                        ...
                        user_id=current_user_id # MODIFIED: Assign user_id
                    )
                    db.session.add(new_session)
                    db.session.commit()
                    logging.info(f"Saved clap session to DB: Start={start_time}, End={end_time}, Claps={count}, UserID={current_user_id}") # MODIFIED LOG
                    save_success = True
            else:
...
def event_history_page():
    ...
    if 'fr_username' in session:
        ...
        if user:
            current_user_id = user.id
            logging.info(f"Event history page loaded for user ID: {current_user_id}")
        else:
            # User in session but not in DB? Should ideally not happen.
            logging.warning(f"User '{session['fr_username']}' in session but not found in DB for event history.")
            flash('User session error. Please log in again.', 'danger')
            return redirect(url_for('login_fr_page')) # Redirect if user context is broken
    else:
        # No user logged in, decide behavior. For now, show unassigned events or redirect.
        # Let's show events not assigned to any user (user_id IS NULL)
        # OR, redirect to login if history should always be user-specific.
        # For simplicity now, let's require login to see event history.
        flash('Please log in to view your event history.', 'info')
        return redirect(url_for('login_fr_page'))

    # Query FallEvents and ClapSessions for the current user
    fall_events_query = FallEvent.query
    clap_sessions_query = ClapSession.query

    if current_user_id:
        # Filter by user_id if a user is logged in
        # Also include events where user_id is NULL (e.g., file-processed falls)
        fall_events = fall_events_query.filter(
            (FallEvent.user_id == current_user_id) | (FallEvent.user_id == None)
        ).order_by(FallEvent.timestamp.desc()).all()
        clap_sessions = clap_sessions_query.filter_by(user_id=current_user_id).order_by(ClapSession.end_time.desc()).all()
    else:
        # If for some reason we want to show truly public/unassigned data (not current logic path)
        fall_events = fall_events_query.filter_by(user_id=None).order_by(FallEvent.timestamp.desc()).all()
        clap_sessions = clap_sessions_query.filter_by(user_id=None).order_by(ClapSession.end_time.desc()).all()
    ...
    for event in fall_events:
        is_users_event_or_unassigned = (event.user_id == current_user_id) or (event.user_id is None and event.source.lower().startswith("file"))
        if not is_users_event_or_unassigned and current_user_id: # Double check, should be handled by query
            continue
        ...
        combined_events.append({
            ...
            'is_own_event': event.user_id == current_user_id, # MODIFIED: Flag if it's user's own
            'is_unassigned_file_event': event.user_id is None and event.source.lower().startswith("file")
        })

    for session_item in clap_sessions: # Renamed to avoid conflict with Flask's session
        if session_item.user_id != current_user_id and current_user_id: # Double check
             continue
        combined_events.append({
            'type': 'clap_session',
            'id': session_item.id,
            'display_timestamp': session_item.end_time, 
            'timestamp_formatted': session_item.end_time.strftime("%Y-%m-%d %H:%M:%S UTC"),
            'clap_count': session_item.clap_count,
            'target_claps': session_item.target_claps,
            'duration_seconds': session_item.duration_seconds,
            'calories_burned': session_item.calories_burned,
            'source': 'Buddha Clap Tracker',
            'is_own_event': True # Clap sessions are always user-specific if queried this way
        })
    ...
    return render_template('event_history.html', events=combined_events, logged_in_user_id=current_user_id)
# --- >>> END NEW <<< ---
...
def complete_registration_fr():
    ...
    with face_rec_state_lock: # Ensure exclusive access
        if 'username' not in temp_registration_data or not registration_process_state_fr.get("captured_embeddings_for_user"):
            # This case is for safety, client UI should prevent submission without data
            return jsonify({'status': 'error', 'message': 'Registration session or embeddings missing.'}), 400

        reg_data = temp_registration_data 
        embeddings_list = registration_process_state_fr.get("captured_embeddings_for_user", [])
        
        if not embeddings_list:
            registration_process_state_fr['status'] = "error_final"
            registration_process_state_fr['error_message'] = "No embeddings captured."
            return jsonify({'status': 'error', 'message': 'No embeddings were captured during registration.'}), 400

        try:
            avg_embedding_np = np.mean(np.array(embeddings_list), axis=0)
            avg_embedding_list = avg_embedding_np.tolist()
        except Exception as e:
            logging.error(f"FR: Error averaging embeddings for {reg_data.get('username', 'UNKNOWN_USER')}: {e}", exc_info=True)
            registration_process_state_fr['status'] = "error_final"
            registration_process_state_fr['error_message'] = "Could not process embeddings."
            return jsonify({'status': 'error', 'message': 'Error processing face data. Please try registration again.'}), 500
        ...
        try:
            ...
            registration_process_state_fr = { 
                ...
                "instruction": "Please start camera for a new registration.", "error_message": None, 
            ...
            return jsonify({
                ...
                'redirect_url': url_for('login_fr_page') # Provide URL for client-side redirect
            }), 200

        except IntegrityError: 
            db.session.rollback()
            logging.warning(f"FR: IntegrityError during final registration for {reg_data['username']}, username likely already exists.")
            return jsonify({'status': 'error', 'message': 'Username already exists. This should have been caught earlier.'}), 409 # Conflict
        except Exception as e:
            db.session.rollback()
            logging.error(f"FR: Error saving user {reg_data['username']} to database: {e}", exc_info=True)
            return jsonify({'status': 'error', 'message': 'An error occurred while saving your registration. Please try again.'}), 500
...
def initialize_app(current_app_instance):
    with current_app_instance.app_context():
        ...
        # --- >>> IMPROVED ASL INITIALIZATION SECTION <<< ---
        initialize_asl_detector()  # This function now handles all detailed logging and error handling internally
        # --- >>> END OF ASL INITIALIZATION SECTION <<< ---
